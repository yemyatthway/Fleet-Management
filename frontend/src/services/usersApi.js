...
const { request, baseUrl } = createApiClient(
  import.meta.env.VITE_USERS_API_BASE_URL || DEFAULT_API_BASE_URL,
);
...
const buildUserFormData = (user) => {
  ...
  if (user.nrcFrontFile instanceof File)
    formData.append("nrcFrontFile", user.nrcFrontFile);
  if (user.nrcBackFile instanceof File)
    formData.append("nrcBackFile", user.nrcBackFile);
