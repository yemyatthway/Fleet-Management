...
const { request, baseUrl } = createApiClient(
  import.meta.env.VITE_VEHICLES_API_BASE_URL || DEFAULT_API_BASE_URL,
);
...
const buildVehicleFormData = (vehicle) => {
  ...
  if (vehicle.vehicleImageFile instanceof File)
    formData.append("vehicleImageFile", vehicle.vehicleImageFile);
  if (vehicle.driverImageFile instanceof File)
    formData.append("driverImageFile", vehicle.driverImageFile);
