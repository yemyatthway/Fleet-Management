...
const { request } = createApiClient(
  import.meta.env.VITE_INVENTORY_PARTS_API_BASE_URL || DEFAULT_API_BASE_URL,
);
...
const buildPartFormData = (part) => {
  ...
  if (part.imageFile instanceof File)
...
export const createInventoryPart = (payload) =>
  request("/api/inventory-parts", {
    method: "POST",
    body: buildPartFormData(payload),
  });
export const updateInventoryPart = (id, payload) =>
  request(`/api/inventory-parts/${id}`, {
    method: "PUT",
    body: buildPartFormData(payload),
  });
