    ...
    <main class="records-page">
      <header class="records-header">
        <div>
          <h1>Reports</h1>
          <p>
            Filter fleet records and export vehicle, trip, maintenance, driver,
            and expense reports.
          </p>
        </div>
      ...
      <section class="summary-grid">
        ...
        <div class="summary-card">
          ...
          <strong>{{
            reportType === "expenses" ? formatCurrency(totalAmount) : "-"
          }}</strong>
        </div>
      </section>
...
const reportOptions = computed(() =>
  isDispatcher.value
    ? allReportOptions.filter(
        (option) => option.value === "vehicles" || option.value === "trips",
      )
    : allReportOptions,
);
...
const columns = computed(() =>
  (columnMap[reportType.value] || []).map(([key, label]) => ({ key, label })),
);
...
const activeReportName = computed(
  () => reportNames[reportType.value] || "Report",
);
...
const formatMyanmarDateTime = (value) => {
  ...
  return Number.isNaN(date.getTime())
    ? "-"
    : myanmarDateTimeFormatter.format(date);
};
