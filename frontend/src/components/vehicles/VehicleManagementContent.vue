<template>
  <div class="vehicle-page">
    <div class="page-header">
      <div>
        ...
        <p class="section-subtitle">
          Track, assign, and maintain your fleet in one place
        </p>
      </div>
    </div>
    ...
    <div v-if="formError && !formOpen" class="form-error page-error">
      {{ formError }}
    </div>
            ...
            <div class="vehicle-cell">
              ...
              >
                <img
                  ...
                  class="vehicle-image"
                />
                <span v-else class="vehicle-image image-placeholder"
                  ><v-icon icon="mdi-truck-outline" size="20"
                /></span>
                <span v-if="item.image" class="tooltip-text"
                  >View vehicle image</span
                >
              </button>
              <div>
            ...
            <div class="driver-cell">
              ...
              >
                <img
                  ...
                  class="driver-photo"
                />
                <span v-else class="driver-photo image-placeholder"
                  ><v-icon icon="mdi-account-outline" size="18"
                /></span>
                <span v-if="item.driverImage" class="tooltip-text"
                  >View driver image</span
                >
              </button>
        ...
        <div class="details-grid">
          <div class="details-section">
            ...
            <div class="details-row">
              <span>Region</span><strong>{{ selectedVehicle.region }}</strong>
            </div>
            <div class="details-row">
              <span>Driver</span><strong>{{ selectedVehicle.driver }}</strong>
            </div>
            <div class="details-row">
              <span>Depot</span><strong>{{ selectedVehicle.depot }}</strong>
            </div>
            <div class="details-row">
              <span>Status</span><strong>{{ selectedVehicle.status }}</strong>
            </div>
            <div class="details-row">
              <span>Capacity</span
          ...
          <div class="details-section">
            ...
            <div class="details-row">
              <span>Acquired</span
              ><strong>{{ formatDate(selectedVehicle.acquiredDate) }}</strong>
            </div>
          </div>
          ...
          <div class="details-section">
            ...
            <div class="details-row">
              ...
              ><strong>{{ selectedVehicle.purchaseCost || "—" }}</strong>
            </div>
            <div class="details-row">
              <span>Fuel Capacity</span
              ><strong>{{ selectedVehicle.fuelCapacity || "—" }}</strong>
            </div>
          </div>
          ...
          <div class="details-section">
            ...
            <div class="details-row">
              ...
              ><strong>{{ selectedVehicle.registrationNo || "—" }}</strong>
            </div>
            <div class="details-row">
            ...
            <div class="details-row">
              <span>Road Tax Expiry</span
              ><strong>{{ formatDate(selectedVehicle.roadTaxExpiry) }}</strong>
            </div>
            <div class="details-row">
            ...
            <div class="details-row">
              ...
              ><strong>{{ selectedVehicle.insuranceProvider || "—" }}</strong>
            </div>
            <div class="details-row">
              <span>Policy No.</span
              ><strong>{{ selectedVehicle.insurancePolicy || "—" }}</strong>
            </div>
            <div class="details-row">
              <span>Inspection Due</span
              ><strong>{{ formatDate(selectedVehicle.inspectionDue) }}</strong>
            </div>
          </div>
          ...
          <div class="details-section">
            ...
            <div class="details-row">
              ...
              ><strong>{{ formatDate(selectedVehicle.lastService) }}</strong>
            </div>
            <div class="details-row">
              <span>Next Service</span
              ><strong>{{ formatDate(selectedVehicle.nextService) }}</strong>
            </div>
            <div class="details-row">
              <span>Service Note</span
              ><strong>{{ selectedVehicle.serviceNote }}</strong>
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-if="formStep === 1" class="form-grid">
          ...
          <div class="form-field">
            ...
            <select v-model="formData.type">
              ...
              <option
                v-for="type in availableVehicleTypeOptions"
                :key="type"
                :value="type"
              >
                {{ type }}
          ...
          <div class="form-field">
            ...
            <select v-model="formData.driver" @change="syncSelectedDriverImage">
              ...
              <option
                v-for="driver in availableDriverOptions"
                :key="driver.name"
                :value="driver.name"
              >
                {{ driver.name }}
          ...
          <div class="form-field">
            ...
            <select v-model="formData.depot">
              ...
              <option
                v-for="depot in availableDepotOptions"
                :key="depot"
                :value="depot"
              >
                {{ depot }}
          ...
          <div class="form-field">
            ...
            <select v-model="formData.fuelType">
              ...
              <option
                v-for="type in availableFuelTypeOptions"
                :key="type"
                :value="type"
              >
                {{ type }}
          ...
          <div class="form-field">
            ...
            <div class="file-row">
              <input
                ...
                type="file"
                accept="image/*"
                @change="handleDriverImageUpload"
              />
              <button
...
const canGoNext = computed(() =>
  formStep.value === 1 ? stepOneValid.value : true,
);
...
const availableDriverOptions = computed(() =>
  driverOptions.value.some((driver) => driver.name === formData.value.driver) ||
  !formData.value.driver
    ? driverOptions.value
    : [
        { name: formData.value.driver, avatar: formData.value.driverImage },
        ...driverOptions.value,
      ],
);
...
const loadReferenceOptions = async () => {
  const [fuelTypes, vehicleTypes, statuses, drivers, depots] =
  ...
  if (fuelTypes.status === "fulfilled" && fuelTypes.value.length)
    fuelTypeOptions.value = fuelTypes.value;
  if (vehicleTypes.status === "fulfilled")
    vehicleTypeOptions.value = vehicleTypes.value;
  ...
  const rejected = [fuelTypes, vehicleTypes, statuses, drivers, depots].find(
    (result) => result.status === "rejected",
  );
  if (rejected) {
    console.error(
      "[vehicles] failed to load reference options",
      rejected.reason,
    );
  }
};
...
const nextFormStep = () => {
  if (formStep.value < formSteps.length && canGoNext.value) {
    ...
    formError.value =
      "Plate, region, type, model, status, driver, and fuel type are required to continue.";
  }
};
...
const syncSelectedDriverImage = () => {
  const selectedDriver = driverOptions.value.find(
    (driver) => driver.name === formData.value.driver,
  );
...
const saveForm = async () => {
  if (
    ...
    formError.value =
      "Plate, region, type, model, status, driver, and fuel type are required.";
    formStep.value = 1;
    showPageMessage({
      tone: "error",
  ...
  try {
    ...
    if (formMode.value === "add") {
      ...
      if (selectedVehicle.value?.id === savedVehicle.id)
        selectedVehicle.value = savedVehicle;
    }
