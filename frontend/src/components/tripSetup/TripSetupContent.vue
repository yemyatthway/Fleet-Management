<template>
  <div class="role-page">
    ...
    <div class="card-surface toolbar">
      <div class="toolbar-row">
        <div class="toolbar-search">
          ...
          <input v-model="searchQuery" type="text" :placeholder="`Search ${label.toLowerCase()} name, code, or description...`" />
          <button v-if="searchQuery" class="clear-button" type="button" aria-label="Clear search" @click="searchQuery = ''">
            <v-icon icon="mdi-close-circle" size="18" />
          </button>
        </div>

        <button v-if="canCreateSetup" class="primary-button" type="button" @click="openAdd">
      ...
      <div class="toolbar-count text-muted">
        {{ loading ? `Loading ${label.toLowerCase()}...` : `Showing ${items.length} of ${total} ${label.toLowerCase()} records` }}
      </div>
    </div>

    <div v-if="pageMessage.message" class="message-section">
      <PageMessage :tone="pageMessage.tone" :title="pageMessage.title" :message="pageMessage.message" @close="clearPageMessage" />
    </div>
    ...
    <FuelTypeSetupDialog :open="dialogOpen" :mode="dialogMode" :item="selectedItem" :label="label" @close="dialogOpen = false" @save="handleSave" />
...
const tableItems = computed(() =>
  attachDisplayIds(items.value, tableOptions.value.page, tableOptions.value.itemsPerPage, false, () => props.codePrefix, {
    total: total.value,
...
const handleSave = async (payload) => {
  try {
    if (dialogMode.value === 'edit') {
      ...
      showPageMessage({ tone: 'success', title: `${props.label} updated`, message: `${payload.name} was updated.` })
    } else {
      await props.api.create(payload)
      showPageMessage({ tone: 'success', title: `${props.label} created`, message: `${payload.name} was created.` })
    }
    dialogOpen.value = false
    await loadItems()
  } catch (error) {
    showPageMessage({ tone: 'danger', title: `Could not save ${props.label.toLowerCase()}`, message: error.message })
  }
}
...
const handleDelete = (item) => {
  openConfirm({
    ...
    action: async () => {
      ...
      showPageMessage({ tone: 'success', title: `${props.label} deleted`, message: `${item.name} was removed.` })
      await loadItems()
    }
  })
}
