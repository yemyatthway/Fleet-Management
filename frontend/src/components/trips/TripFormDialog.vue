          ...
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="field">
                ...
                <span v-if="driverAssignmentHint" class="field-hint warning">{{
                  driverAssignmentHint
                }}</span>
              </div>
              <div class="field">
              ...
              <div class="field">
                ...
                <select v-model="form.dispatcherName" required>
                  ...
                  <option
                    v-for="dispatcher in dispatcherOptions"
                    :key="dispatcher.name"
              ...
              <div class="field">
                ...
                <select v-model="form.cargoType" required>
                  ...
                  <option
                    v-for="cargoType in cargoTypeOptions"
                    :key="cargoType"
                    :value="cargoType"
                  >
                    {{ cargoType }}
              ...
              <div class="field">
                ...
                <input
                  ...
                  required
                />
                <span v-if="capacityText" class="field-hint"
                  >Vehicle capacity: {{ capacityText }}</span
                >
              </div>
              <div class="field">
                ...
                />
                <span v-if="capacityText" class="field-hint"
                  >Keep volume within the selected vehicle capacity.</span
                >
              </div>
            </div>
          </div>
          ...
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="field checkbox-field">
                <label
                  ><input
                    v-model="form.temperatureControlled"
                    type="checkbox"
                  />
...
const selectedVehicle = computed(
  () =>
    props.vehicleOptions.find(
      (vehicle) => vehicle.id === props.form.vehicleId,
    ) || null,
);

const assignedDriver = computed(() =>
  String(selectedVehicle.value?.driver || "").trim(),
);

const capacityText = computed(() => selectedVehicle.value?.capacity || "");

const driverAssignmentHint = computed(() => {
  if (props.form.vehicleId && !assignedDriver.value)
    return "Assign a driver to this vehicle before creating a trip.";
  if (
    props.form.vehicleId &&
    props.form.driverName &&
    assignedDriver.value.toLowerCase() !==
      String(props.form.driverName).trim().toLowerCase()
  ) {
    return `This vehicle is assigned to ${assignedDriver.value}.`;
  }
  if (props.form.driverName && !props.form.vehicleId)
    return "Select a driver to auto-fill their assigned vehicle.";
  return "";
});
...
const syncVehicle = () => {
  const selectedVehicle = props.vehicleOptions.find(
    (vehicle) => vehicle.id === props.form.vehicleId,
  );
  if (!selectedVehicle) return;
  props.form.vehiclePlate = selectedVehicle.plate || "";
  props.form.driverName = selectedVehicle.driver || "";
  props.form.startingOdometerKm =
    Number(String(selectedVehicle.odometer || "").replace(/[^\d.]/g, "")) ||
    props.form.startingOdometerKm;
  props.form.currentOdometerKm =
    props.form.startingOdometerKm || props.form.currentOdometerKm;
};

const syncDriver = () => {
  const selectedDriverName = String(props.form.driverName || "")
    .trim()
    .toLowerCase();
  ...
  const assignedVehicle = props.vehicleOptions.find(
    (vehicle) =>
      String(vehicle.driver || "")
        .trim()
        .toLowerCase() === selectedDriverName,
  );
