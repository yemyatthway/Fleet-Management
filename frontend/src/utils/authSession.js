...
export const getPermission = (moduleKey) =>
  getAuthSession()?.permissions?.find(
...
const isAdminSession = () => {
  ...
  const role = String(
    session?.user?.role || session?.user?.roleId || "",
  ).toLowerCase();
  return role === "admin";
};
...
export const canViewModule = (moduleKey) => {
  ...
  if (isDispatcherSession() && String(moduleKey).endsWith("-setup"))
    return false;
...
const isDispatcherSession = () => {
  const session = getAuthSession();
  const role = String(
    session?.user?.roleId || session?.user?.role || "",
  ).toLowerCase();
